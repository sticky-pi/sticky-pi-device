...
class DataSyncer(object):
    ...
    def __init__(self, user_requested = False, periodic=False, battery_level=None, no_files = False, dev_id = None):
        ...
        self._logfile_path = os.path.join(self._config.SPI_IMAGE_DIR, self._config.SPI_LOG_FILENAME)
    ...
    def sync(self):
        ...
        while not self._is_any_net_interface_up():
            ...
            if time.time() - start > timeout:
                ...
                # so we have time to display the message
                if self._interface_timeout_long:
                    time.sleep(1)
                return
    ...
    def _upload_images(self):
        ...
        # so we can reeuse the same connection
        with requests.Session() as session:
            ...
            response = session.post(url_images_to_upload, payload, headers=headers)
            ...
            def upload_one_image(image):
                ...
                if image_status[image] != "uploaded":
                    ...
                    with open(image_path, 'rb') as f:
                        ...
                        response = session.post(url_upload, files=payload)
            ...
            with Pool(self._upload_pool_size) as p:
                p.map(upload_one_image, sorted(image_status.keys()))
            ...
            with open(self._logfile_path, 'rb') as f:
                ...
                session.post(url_upload_log, files=payload)

            # Formally finish transaction
            device_status['in_transaction'] = 0
            response = session.post(url_update_status, json=device_status)
            assert response.status_code == 200, response
